  ...
  {/if}
       ...
       {#if dashboard_state.all_notes.length > 0}
        <ul>
          {#each dashboard_state.all_notes as note}
            <li class="mb-2 cursor-pointer rounded-md p-2 hover:bg-slate-600 dark:hover:bg-slate-500">
	...
	<div class="flex w-full flex-row justify-end p-4">
      ...
      onclick={() => (dashboard_state.new_note_modal = true)}
		...
		>
    <Modal title="New note" class="dark" bind:open={dashboard_state.new_note_modal} onaction={({ action }) => alert(`Handle "${action}"`)}>
      <div>
        <input 
          type="text" 
          name="title" 
          ...
          dark:bg-slate-800 dark:text-slate-200 dark:focus:border-slate-400" bind:value={dashboard_state.new_note.title} />
      </div>
      <div class="w-full h-full bg-slate-100 rounded-sm overflow-hidden">
       <Editor
        class="h-96"
        options={dashboard_state.quill.options}
        {onTextChange}
      >{@html dashboard_state.quill.content.html}</Editor>
      </div>
      <div class="w-full p-2">
        <button class="btn btn-block py-2 px-4 bg-sky-400 hover:bg-sky-500 w-full rounded-md shadow-md text-white cursor-pointer"
          disabled={dashboard_state.new_note.title === "" || dashboard_state.quill.content.text === ""}
